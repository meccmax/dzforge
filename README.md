...
| 🗺️ **Map & quests** | Live map of trader zones, quest NPCs, objectives, AI patrols, airdrops, contaminated areas, event spawns, territories, turrets and DNA keycard crates/vaults. Drag markers to move, place new entities (incl. keycard crates/vaults by tier and **AI missions/camps** with bot count, loadout and range), edit or clear them in popups. Plus a **"scan every mod & profile for coordinates"** layer that plots anything coordinate-based (AI missions, patrols, spawners, safe zones…) as colour-coded, mission-named markers grouped and toggleable per mod. |
...
| 🪙 **Traders** | Per-zone health **Verify** and a stock/price editor (buy/sell %, radius, searchable item list), **plus a full Expansion Market catalog editor** — tune item prices and **bulk-import items from your economy by category with auto-suggested prices** (a built-in Types→Market converter). |
| 🎖️ **Loadouts** | DayZ Expansion loadouts via a visual paper-doll slot editor, copy/paste items, search, verify, and **export/import as shareable bundles**. |
| 📜 **Quests** | A **Quest Wizard** builds a complete, working quest — quest + objective + NPC, all linked — from a few plain-English questions, hiding the underlying files and IDs. Plus the full editor (story/dialogue, rewards, reputation, prerequisites, NPC givers, objectives), dedicated **Objective**/**NPC** editors, a **plain-language summary** on every quest ("Talk to X → kill 8 wolves → reward Y"), a server-wide **clickable audit**, and **export/import of shareable quest bundles** (objectives + NPCs travel with the quest; IDs are remapped on import). |
